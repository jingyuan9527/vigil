...
| POST | `/api/scan?force=1` | 立即触发一次扫描；`force=1` 为强制扫描，对所有存在版本差异的镜像重新补发通知（每次强制扫描都会重新广播） |
...
- 首巡会把当时已存在的全部 tag 记为已见基线（不通知）；这些「基线吞掉」的更高版本可由**强制扫描**按版本号比对找回（见下文「强制扫描」）。
...
- new-tag：每个 tag 仅通知一次；首扫建立已见标签基线，不刷屏；**强制扫描除外**——按版本号重播「更高版本 tag」提醒时无视基线与已读，每次触发都会再次通知。

### 忽略优先级最高
镜像被忽略后**跳过全部检测**（不校验摘要、不巡检标签），行数据冻结，也不产生任何通知。

### 强制扫描
更新通知页「全部重新扫描」触发强制扫描：无视去重与已读，对所有存在版本差异的镜像重新广播 `update` 通知（含 Pin-Watch 锁定 tag 被覆盖的情况）。
强制扫描语义为**重新广播**：把所有镜像当成从未通知过来重新判定差异，每次触发都会再次通知（系统 + 钉钉），适合运维复盘或清空已读历史后找回提醒。
- 本地镜像（Docker 采集）：差异基线为本地摘要，本地 ≠ 远端即重新广播。
- 纯远端监控（manual / watch）：常规扫描的上一轮远端会被覆盖，强制扫描改从版本时间线取最早记录的摘要作为基线，仅当与当前远端不同才重新广播（即「自首次记录以来的转移」）；远端从未变化、或已回到首次记录时无可广播内容。
- Pin-Watch 镜像：按版本号比对仓库完整 tag 列表，存在比锁定 tag 更高的版本 → 重播最新的一个 `new-tag` 通知（含首巡基线吞掉的更高版本，如锁定 `8.4.5` 而仓库已有 `26`）；该 tag 已在本次扫描中通知过则不重复；仓库无更高版本 tag 时不重播。
